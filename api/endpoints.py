...
def initialize(app_):
    ...
    def get_team_scores_html(table_id: int, style: Optional[str] = Query(None, max_length=15)):
    ...
    def get_full_table_html(table_id: int, style: Optional[str] = Query(None, max_length=15)):
        table_bot = cb_interface.get_table_bot(table_id)
        if table_bot is None:
            raise HTTPException(status_code=404, detail="Table ID not found. Either the table has been reset, or the table ID given does not exist. Table Bot gave you a table ID when you started the table. Use that.")
        return api_data_builder.build_full_table_html(cb_interface.get_team_score_data(table_bot), style)
