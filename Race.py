...
class Race:
    ...
    def track_check(self):
        if len(self.track) > 0 and UtilityFunctions.is_hex(self.track):
            common.log_error(
                f"The following track had no SHA mapping: {self.track}")
    ...
    def getRoomRating(self):
        roomRating = 0
        all_ratings = [placement.get_player().get_player_skill_rating()
                       for placement in self.placements]
    ...
    def insertPlacement(self, old_position_number, new_position_number):
        self.placements.insert(new_position_number-1,
                               self.placements.pop(old_position_number-1))
...
Track: {self.track}
CC: {self.cc}
Number of placements: {len(self.placements)}
Region: {self.region}
Is CT: {self.is_ct}
Is Wiimmfi RaceD: {self.is_wiimmfi_race}
MKWX Race Number: {self.mkwxRaceNumber}"""

    def __str__(self):
        curStr = "Race #" + str(self.raceNumber) + " - " + UtilityFunctions.filter_text(self.getTrackNameWithoutAuthor()) + " - " + str(self.cc) + "cc" + \
