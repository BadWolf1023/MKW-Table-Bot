                ...
                if ((int(race.raceNumber)-1) % 4) == 0:
                    ...
                    for missingFC, missingName in missingPlayersByRace[int(race.raceNumber)-1]:
                        ...
                        if race.raceNumber in dc_on_or_before\
                            ...
                            race_errors[int(race.raceNumber)].append(missingName + UserDataProcessing.lounge_add(missingFC, lounge_replace) + " is missing. Giving " + str(war.missingRacePts) + " DC points per missing race. (" + str(len(race.placements)) + " players in room) - Use ?dcs to fix this.")
